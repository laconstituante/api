...
//    }
    public function resample($width, $height, $constrainProportions = true)
    {
        ...
        if ($constrainProportions) {
            if ($this->height > $this->width) {
                //$width  = round($height / $this->height * $this->width);
                $this->crop(0, round(($this->height - $this->width)/2), $this->width,round(($this->height + $this->width)/2));
            } else if($this->height < $this->width) {
                //$height = round($width / $this->width * $this->height);
                $this->crop(round(($this->width - $this->height)/2),0,  round(($this->width + $this->height)/2),$this->height);
            }
        }
    ...
    public function customResample($width, $height, $constrainProportions = true)
    {
        $decalageVertical = true;
        if (!is_resource($this->image)) {
            throw new RuntimeException('No image set');
        }
        if ($constrainProportions) {
            if ($this->height < $this->width) {
                $decalageVertical = false;
            }
        }
        if($decalageVertical){
            //return $this->_replace($temp);
            $temp = imagecreatetruecolor($this->width, $this->width);
            imagecopy($temp, $this->image, 0, 0, 0, round(($this->height - $this->width)/2),$this->width, round(($this->height + $this->width)/2));            
            return $this->_replace($temp);
        }
        else{
            $temp = imagecreatetruecolor($this->height, $this->height);
            imagecopy($temp, $this->image, 0, 0, round(($this->width - $this->height)/2),0,$this->height, round(($this->height + $this->width)/2)); 
            return $this->_replace($temp);
        }
...
//    }
    public function save($fileName, $type = IMAGETYPE_PNG)
    {
        ...
        try {
            ...
            //TODO LOG throw new RuntimeException('Error saving image file to ' . $fileName);
            return false;
        }
        return true;
    }
